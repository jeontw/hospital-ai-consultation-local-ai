...
> 병원 전화 상담 내용을 로컬 AI로 분석하고, 환자별 상담 이력과 예약 일정을 한 화면에서 관리하는 웹 애플리케이션입니다.

음성 상담 파일 또는 간호 메모를 입력하면 `whisper.cpp`가 음성을 텍스트로 변환하고, `Ollama` 기반 로컬 LLM이 상담 요약, 증상, 위험도, 키워드, 환자 후보, 예약 초안을 생성합니다. 외부 AI API로 상담 데이터를 보내지 않는 로컬 처리 구조를 목표로 합니다.
...
- 음성 파일 업로드 기반 상담 등록
- 간호 메모 기반 상담 등록
- AI 상담 미리보기 후 환자와 예약 정보를 검토하여 확정 저장
...
- `Ollama` 기반 로컬 LLM 분석
- 지원 모델 선택: `qwen2.5:3b`, `qwen2.5:7b`, `exaone3.5:7.8b`
- 상담 내용 기반 환자 후보 추출
- 진료 전 참고용 의사 브리핑 생성
...
프로젝트에서 허용하는 모델 중 사용할 모델을 내려받습니다.
...
`application.properties`의 `whisper.exe-path`, `whisper.model-path` 값을 실제 설치 경로에 맞게 수정해야 합니다.

### FFmpeg 준비

현재 백엔드 코드는 아래 경로의 FFmpeg 실행 파일을 사용합니다.
...
MySQL에는 `consultation_db` 데이터베이스를 미리 생성해야 합니다.
...
### Ollama 응답이 비어 있거나 AI 분석이 실패하는 경우

- `ollama list`로 모델이 설치되어 있는지 확인합니다.
- `application.properties`의 `ollama.model` 값이 허용 모델 목록에 포함되는지 확인합니다.
...
- 백엔드 CORS 설정의 허용 origin이 프론트엔드 주소와 일치하는지 확인합니다.
