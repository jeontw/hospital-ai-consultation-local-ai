...
function PatientList({
  ...
  const normalizedSearchPhone = normalizeDigits(searchPhone);
  const normalizedSearchBirth = normalizeDigits(searchBirth);
  const hasSearchCondition = Boolean(
    normalizedSearchName || normalizedSearchPhone || normalizedSearchBirth,
  );
  const matchedPatients = hasSearchCondition
    ? patients.filter((patient) => {
        const patientName = normalizeText(patient.name);
        const patientPhone = normalizeDigits(patient.phone);
        const patientBirth = normalizeDigits(patient.birth);
    ...
    : showAllPatients
      ...
      : selectedPatient
        ? [selectedPatient]
      : [];
  const displayedPatients = hasSearchCondition
    ? matchedPatients.slice(0, 30)
    : matchedPatients;
  const hasMoreResults =
    hasSearchCondition && matchedPatients.length > displayedPatients.length;
  ...
  const updateSearchPhone = (value) => {
    setSearchPhone(value);
    setShowAllPatients(false);
  };

  const updateSearchBirth = (value) => {
    setSearchBirth(value);
    setShowAllPatients(false);
  };
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
      ...
      <div className="mb-2 flex flex-col gap-2">
        <div className="grid grid-cols-[1fr_1fr_1fr] gap-2">
          ...
          <input
            type="text"
            value={searchPhone}
            onChange={(e) => updateSearchPhone(e.target.value)}
            className="h-9 w-full rounded-md border border-slate-300 px-3 text-base"
            placeholder="전화번호"
          />
          <input
            type="text"
            value={searchBirth}
            onChange={(e) => updateSearchBirth(e.target.value)}
            className="h-9 w-full rounded-md border border-slate-300 px-3 text-base"
            placeholder="생년월일"
          />
        </div>
        <p className="text-sm text-slate-500">
          입력한 조건을 모두 만족하는 환자만 표시됩니다.
        </p>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-slate-200">
        ...
        {hasSearchCondition || showAllPatients || selectedPatient ? (
